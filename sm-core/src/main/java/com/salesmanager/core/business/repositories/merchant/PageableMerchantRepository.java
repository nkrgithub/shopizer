...
public interface PageableMerchantRepository extends PagingAndSortingRepository<MerchantStore, Long> {
	...
	@Query(value = "select distinct m from MerchantStore m " +
			...
			"left join fetch m.languages mls where (m.storename like %?1% or ?1 is null)", // Modified by nkr
			countQuery = "select count(distinct m) from MerchantStore m where (m.storename like %?1% or ?1 is null)") // Modified by nkr
	...
	@Query(value = "select distinct m from MerchantStore m left join fetch m.parent mp "
			...
			+ "where m.retailer = true and (m.storename like %?1% or ?1 is null)", countQuery = "select count(distinct m) from MerchantStore m join m.parent "
					+ "where m.retailer = true and (m.storename like %?1% or ?1 is null)")
	...
	@Query(value = "select distinct m from MerchantStore m left join m.parent mp " + "left join fetch m.country pc "
			...
			+ "and ((m.storename like %?2% or mp.storename like %?2%) or ?2 is null)", countQuery = "select count(distinct m) from MerchantStore m left join m.parent mp "
					+ "where mp.code = ?1 or m.code = ?1 and ((m.storename like %?2% or mp.storename like %?2%) or ?2 is null)")
	Page<MerchantStore> listChilds(String storeCode, String storeName, Pageable pageable);

	@Query(value = "select * from MERCHANT_STORE m " + "where (m.STORE_CODE = ?1 or (m.PARENT_ID = ?2 or ?2 is null)) "
			+ "and (m.STORE_NAME like %?3% or ?3 is null)", countQuery = "select count(*) from {h-schema}MERCHANT_STORE m where (m.STORE_CODE = ?1 or (?2 is null or m.PARENT_ID = ?2)) and (?3 is null or m.STORE_NAME like %?3%)", nativeQuery = true)
